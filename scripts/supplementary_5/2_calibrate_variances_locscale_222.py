...
def main():    
    ...
    # shuffle the data
    shuffled_indices_emdb = np.random.permutation(len(residuals_list))
    residuals_list = [residuals_list[i] for i in shuffled_indices_emdb]
    masked_variance_list = [masked_variance_list[i] for i in shuffled_indices_emdb]

    num_emdb_train = int(len(residuals_list) * 0.6)
    # residuals_train = residuals_list[:num_emdb_train]
    # masked_variance_train = masked_variance_list[:num_emdb_train]
    # residuals_test = residuals_list[num_emdb_train:]
    # masked_variance_test = masked_variance_list[num_emdb_train:]
    ...
    # residuals_array_train = np.concatenate(residuals_train)
    # masked_variance_array_train = np.concatenate(masked_variance_train)
    # residuals_array_test = np.concatenate(residuals_test)
    # masked_variance_array_test = np.concatenate(masked_variance_test)
    ...
    # standard_errors_train = np.sqrt(masked_variance_array_train) / np.sqrt(num_samples)
    # absolute_residuals_train = np.abs(residuals_array_train)
    # standard_errors_test = np.sqrt(masked_variance_array_test) / np.sqrt(num_samples)
    # absolute_residuals_test = np.abs(residuals_array_test)

    X = np.array(standard_errors)
    Y = np.array(absolute_residuals)

    #print(f"Min standard error: {X.min()}, Max standard error: {X.max()}")
    #print(f"Min absolute residual: {Y.min()}, Max absolute residual: {Y.max()}")
    ...
    # X_train = standard_errors_train
    # Y_train = absolute_residuals_train
    # X_test = standard_errors_test
    # Y_test = absolute_residuals_test
