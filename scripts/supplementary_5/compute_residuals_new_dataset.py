...
def main():    
    ...
    feature_enhance_folder = "/home/abharadwaj1/papers/elife_paper/figure_information/archive_data/organized/rebuttal/larger_dataset/calibration"
    ...
    output_folder = os.path.join(data_archive_path, "processed", "general", "supplementary_5", "residuals_analysis_new_dataset")
    ...
    output_filename = os.path.join(output_folder, "residuals_analysis_new_dataset.pickle")
    emdb_pdbs = [x for x in os.listdir(feature_enhance_folder) if os.path.isdir(os.path.join(feature_enhance_folder, x)) and x.startswith("emd_")]
    ...
    for emdb_pdb in emdb_pdbs:
        ...
        mean_prediction_path = os.path.join(emdb_prediction_folder, f"emd_{emdb}_fe.mrc")
        var_prediction_path = os.path.join(emdb_prediction_folder, "processing_files", f"emd_{emdb}_fe_variance.mrc")
        mask_path = os.path.join(emdb_prediction_folder, "processing_files", f"EMD_{int(emdb)}_unsharpened_fullmap_confidenceMap.mrc")
        locscale_map_path = os.path.join(emdb_prediction_folder, f"emd_{emdb}_fe_baseline.mrc")
