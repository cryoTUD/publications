...
def main():    
    ...
    filtered_data_path = os.path.join(data_archive_path, "structured_data", "supplementary_2a", "atomic_v_pseudoatomic_adp_correlation_filtered.pickle")
    input_filename = os.path.join(data_input_folder_main, "fsc_curves_test_83.pkl")  # input pickle file
    # other input folder 

    figure_output_folder_main = os.path.join(data_archive_path, "outputs", "figure_2")
    # other output folder
    assert_paths_exist(input_filename, filtered_data_path)  # check if input files exist
    create_folders_if_they_do_not_exist(figure_output_folder_main) # for output files
    
    output_filename = os.path.join(figure_output_folder_main, "fsc_curves_with_fdr_mask_many_maps_83.pdf")  # output plot preferably in pdf format

    # Load the training data features
    with open(input_filename, 'rb') as f:
        fsc_curves_raw = pickle.load(f)

    with open(filtered_data_path, 'rb') as f:
        filtered_data = pickle.load(f)
    print(filtered_data["EMDB_PDB_ids_present"][:3])
    fsc_curves_unfiltered = [f for f in fsc_curves_raw if f is not None]
    
    print(f"Number of FSC curves loaded: {len(fsc_curves_unfiltered)}")
    fsc_curves = [f for f in fsc_curves_unfiltered if f["emdb_pdb"] in filtered_data["EMDB_PDB_ids_present"]]
    print(f"Number of FSC curves after filtering: {len(fsc_curves)}")
    missing_emdb_ids = set(filtered_data["EMDB_PDB_ids_present"]) - set([f["emdb_pdb"] for f in fsc_curves])
    print(f"Missing EMDB-PDB IDs in FSC curves: {missing_emdb_ids}")
