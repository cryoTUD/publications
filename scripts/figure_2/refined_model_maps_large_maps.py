...
from scripts.figure_2.figure_2_functions import compute_fsc_all_cycles, save_results, refine_model, simulate_maps, create_initial_pseudomodel, download_halfmaps
...
# %% GLOBAL VARIABLES
DOWNLOAD_HALFMAPS = True
CREATE_PSEUDOMODEL = True
REFINE_MODEL = True
SIMULATE_MAPS = True
CALCULATE_FSC = True
...
def main():
    ...
    halfmap_folder = os.path.join(data_archive_path, "raw", "maps", "halfmaps")
    confidence_mask_folder = os.path.join(data_archive_path, "raw", "maps", "confidence_masks")
    output_folder = os.path.join(data_archive_path, "processed", "pseudomodel_refinement_overfitting")


    # CREATE OUTPUT FOLDER
    create_folders_if_they_do_not_exist(output_folder)
    sampled_emdb_pdb_ids = [] 

    while len(sampled_emdb_pdb_ids) < 15:
        try: 
            random.seed(42)
            np.random.seed(42)
            emdb_pdb = random.choice(list(set(EMDB_PDB_ids_all) - set(sampled_emdb_pdb_ids)))
            print(f"Processing {emdb_pdb}...")
            output_folder_emdb = os.path.join(output_folder, f"{emdb_pdb}")
            create_folders_if_they_do_not_exist(output_folder_emdb)
            emdb, pdb = emdb_pdb.split("_")
            confidence_mask_path = os.path.join(confidence_mask_folder, f"emd_{emdb}_FDR_confidence_final.map")
            if not os.path.exists(confidence_mask_path):
                print(f"Confidence mask not found for EMD-{emdb}, skipping...")
                continue
            halfmap1_path = os.path.join(halfmap_folder, f"emd_{emdb}_half_1.map")
            halfmap2_path = os.path.join(halfmap_folder, f"emd_{emdb}_half_2.map")
            if DOWNLOAD_HALFMAPS and (not os.path.exists(halfmap1_path) or not os.path.exists(halfmap2_path)):
                halfmap1_path, halfmap2_path = download_halfmaps(emdb, halfmap_folder)

            

            copied_halfmap1_path = os.path.join(output_folder_emdb, os.path.basename(halfmap1_path))
            copied_halfmap2_path = os.path.join(output_folder_emdb, os.path.basename(halfmap2_path))

            if not os.path.exists(copied_halfmap1_path) or not os.path.exists(copied_halfmap2_path):
                copy_file_to_folder(halfmap1_path, output_folder_emdb)
                copy_file_to_folder(halfmap2_path, output_folder_emdb)
            
            copied_mask_path = os.path.join(output_folder_emdb, os.path.basename(confidence_mask_path))
            if not os.path.exists(copied_mask_path):
                copy_file_to_folder(confidence_mask_path, output_folder_emdb)
            
            mask_array, apix = load_map(copied_mask_path)
            if CREATE_PSEUDOMODEL:
                model_path_halfmap_1 = create_initial_pseudomodel(copied_mask_path, halfmap1_path, NUM_ITER_PSEUDO)
            else:
                model_path_halfmap_1 = os.path.join(output_folder_emdb, f"emd_{emdb}_FDR_confidence_final_gradient_pseudomodel.cif")
            
            if not os.path.exists(model_path_halfmap_1):
                print(f"Initial pseudomodel not found for EMD-{emdb}, skipping...")
                continue
            resolution = apix * 2 + 0.1
            if REFINE_MODEL:
                print("Refining model...")
                refined_model_path = refine_model(
                    model_path=model_path_halfmap_1,
                    map_path=copied_halfmap1_path,
                    resolution=resolution,
                    num_iter=NUM_ITER_REFINE
                )

            if SIMULATE_MAPS:
                emmap, apix = load_map(copied_halfmap1_path)
                size = emmap.shape
                print("Simulating maps...")
                simulated_map_paths = simulate_maps(
                    apix = apix, shape=size,
                    output_folder=output_folder_emdb,
                    num_iter=NUM_ITER_REFINE,
                )
            ...
            if CALCULATE_FSC:
                ...
                results = compute_fsc_all_cycles(
                    output_folder=output_folder_emdb, 
                    halfmap1_path=copied_halfmap1_path,
                    halfmap2_path=copied_halfmap2_path,
                ...
                save_results(
                    ...
                    output_folder=output_folder_emdb, 
                    num_iter=NUM_ITER_REFINE)
            sampled_emdb_pdb_ids.append(emdb_pdb)
