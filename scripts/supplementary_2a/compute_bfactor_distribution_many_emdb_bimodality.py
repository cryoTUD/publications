...
def run_tobvalid_and_map_clusters(emdb_pdb, input_files, input_files_MB):
    ...
    # shift bfactors 
    output_folder = "/home/abharadwaj1/papers/elife_paper/figure_information/archive_data/structured_data/supplementary_2a/tobvalid_analysis/albe_results_shifted"
    #shifted_atomic_model, shift_value = shift_bfactors_by_probability(input_pdb=atomic_model_path, probability_threshold=0.01, minimum_bfactor=0)
    #shifted_pseudomodel, _ = shift_bfactors_by_probability(input_pdb=pseudomodel_path, probability_threshold=0.01, minimum_bfactor=0)
    shifted_atomic_model_path = os.path.join(output_folder, f"{emdb}_{pdb}_atomic_model_shifted.mmcif")
    shifted_pseudomodel_path = os.path.join(output_folder, f"{emdb}_{pdb}_pseudomodel_shifted.mmcif")
    #shifted_atomic_model.make_mmcif_document().write_file(shifted_atomic_model_path)
    #shifted_pseudomodel.make_mmcif_document().write_file(shifted_pseudomodel_path)
    if os.path.exists(atomic_model_path):
        # run tobvalid 
        os.system("tobvalid -i {} -m 'auto' -g".format(shifted_atomic_model_path))
        # map the clusters to the pdb file
        # try:
        #     basename_without_extension = os.path.splitext(os.path.basename(atomic_model_path))[0]
        #     tobvalid_directory = os.path.join(os.path.expanduser("~"), basename_without_extension)
        #     atomic_model_directory = os.path.dirname(atomic_model_path)
        #     # cluster_report_path = os.path.join(tobvalid_directory, "cluster\ report.txt")
        #     # move tobvalid directory to the same directory as the atomic_model_path
        #     # shutil.move(tobvalid_directory, atomic_model_directory)
        #     #os.system("python /home/abharadwaj1/common_scripts/map_clusters_to_pdb.py --cluster_report {} --pdb_file {}".format(cluster_report_path, atomic_model_path))
        # except Exception as e:
        #     print(f"Error processing {emdb_pdb}: {e}")
        #     return None
    else:
        print(f"Atomic model path does not exist for {emdb_pdb}: {atomic_model_path}")
        return None
    if os.path.exists(pseudomodel_path):
        # run tobvalid 
        os.system("tobvalid -i {} -m 'auto' -g".format(shifted_pseudomodel_path))
        # map the clusters to the pdb file
        # try:
        #     basename_without_extension = os.path.splitext(os.path.basename(pseudomodel_path))[0]
        #     tobvalid_directory = os.path.join(os.path.expanduser("~"), basename_without_extension)
        #     pseudomodel_directory = os.path.dirname(pseudomodel_path)
        #     cluster_report_path = os.path.join(tobvalid_directory, "cluster\ report.txt")
        #     # move tobvalid directory to the same directory as the pseudomodel
        #     #shutil.move(tobvalid_directory, pseudomodel_directory)
        #     #os.system("python /home/abharadwaj1/common_scripts/map_clusters_to_pdb.py --cluster_report {} --pdb_file {}".format(cluster_report_path, pseudomodel_path))
        # except Exception as e:
        #     print(f"Error processing {emdb_pdb}: {e}")
        #     return None

    else:
        print(f"Pseudomodel path does not exist for {emdb_pdb}: {pseudomodel_path}")
        return None
...
def main():
    ...
    output_filename = os.path.join(output_folder_main, "atomic_v_pseudoatomic_adp_correlation.pickle")  # output plot preferably in json
    ...
    for emdb_pdb in EMDB_PDB_ids_all:
        input_files_model_free = get_input_files_for_adp_correlation_atomicmodel_pseudomodel(emdb_pdb, "MF", averagedPseudomodel=False)
    ...
    bfactor_correlation_emdb = {}
    sample_emdb_pdb = list(input_files_MB.keys())
    bfactor_list_pseudo = {}
    bfactor_list_atomic = {}
    keys_present_in_both = set(input_files.keys()).intersection(set(input_files_MB.keys()))

    # for emdb_pdb in keys_present_in_both:
    ...
    #     if os.path.exists(atomic_model_path):
    #         # run tobvalid 
    #         os.system("tobvalid -i {} -m 2".format(atomic_model_path))
    #         # map the clusters to the pdb file
    #         try:
    #             basename_without_extension = os.path.splitext(os.path.basename(atomic_model_path))[0]
    #             tobvalid_directory = os.path.join(os.path.dirname(atomic_model_path), basename_without_extension)
    #             cluster_report_path = os.path.join(tobvalid_directory, "cluster report.txt")
    #             os.system("python /home/abharadwaj1/common_scripts/map_clusters_to_pdb.py --cluster_report {} --pdb_file {}".format(cluster_report_path, atomic_model_path))
    #         except Exception as e:
    #             print(f"Error processing {emdb_pdb}: {e}")
    ...
    #     if os.path.exists(pseudomodel_path):
    #         # run tobvalid 
    #         os.system("tobvalid -i {} -m 2".format(pseudomodel_path))
    #         # map the clusters to the pdb file
    #         try:
    #             basename_without_extension = os.path.splitext(os.path.basename(pseudomodel_path))[0]
    #             tobvalid_directory = os.path.join(os.path.dirname(pseudomodel_path), basename_without_extension)
    #             cluster_report_path = os.path.join(tobvalid_directory, "cluster report.txt")
    #             os.system("python /home/abharadwaj1/common_scripts/map_clusters_to_pdb.py --cluster_report {} --pdb_file {}".format(cluster_report_path, pseudomodel_path))
    #         except Exception as e:
    #             print(f"Error processing {emdb_pdb}: {e}")
    ...
    # Parallel processing for atomic and pseudoatomic models
    _ = Parallel(n_jobs=10, verbose=10)(
        delayed(run_tobvalid_and_map_clusters)(emdb_pdb, input_files, input_files_MB)\
            for emdb_pdb in tqdm(keys_present_in_both, desc="Running tobvalid and mapping clusters")
    )
