...
def copy_files_to_folder(file_path, output_folder):
    """
    Copies a file to the specified output folder and returns the new file path.
    """
    assert_paths_exist(file_path, output_folder)
...
def main():
    ...
    output_folder_for_all_refinement = os.path.join(output_folder_main, "bfactor_refinement_all_using_halfmaps")
    create_folders_if_they_do_not_exist(output_folder_main, output_folder_for_all_refinement)
    ...
    for emdb_pdb in EMDB_PDB_ids_all:
        output_folder_emdb_MF = os.path.join(output_folder_for_all_refinement, emdb_pdb, "model_free")
        output_folder_emdb_MB = os.path.join(output_folder_for_all_refinement, emdb_pdb, "model_based")
        output_folder_emdb_hyb = os.path.join(output_folder_for_all_refinement, emdb_pdb, "hybrid")
        create_folders_if_they_do_not_exist(output_folder_emdb_MF, output_folder_emdb_MB, output_folder_emdb_hyb)
        ...
        input_files_model_hybrid = get_input_files_for_adp_correlation_atomicmodel_pseudomodel(emdb_pdb, "hybrid")
        
        required_files = ["target_pdb_path", "halfmap_1_path", "halfmap_2_path"]
        ...
        for required_file in required_files:
            ...
            if not os.path.exists(input_files_model_hybrid[required_file]):
        ...
        if len(missing_files) == 0:
            ...
            emmap, apix = load_map(input_files_model_free["halfmap_1_path"])
            resolution_from_apix = 2*apix + 0.1
            input_files[emdb_pdb] = {}
            for key in required_files:
                # copy files to output folder 
                continue
                input_files[emdb_pdb][key] = copy_files_to_folder(input_files_model_free[key], output_folder_emdb_MF)
            input_files_MB[emdb_pdb] = {}
            for key in required_files:
                continue
                input_files_MB[emdb_pdb][key] = copy_files_to_folder(input_files_model_based[key], output_folder_emdb_MB)
            input_files_hyb[emdb_pdb] = {}
            for key in required_files:
                input_files_hyb[emdb_pdb][key] = copy_files_to_folder(input_files_model_hybrid[key], output_folder_emdb_hyb)
            
            input_files[emdb_pdb]["resolution"] = resolution_from_apix
            input_files_MB[emdb_pdb]["resolution"] = resolution_from_apix
            input_files_hyb[emdb_pdb]["resolution"] = resolution_from_apix

    print(f"Number of EMDB-PDB pairs: {len(emdb_pdbs_present)}")

    from joblib import Parallel, delayed

    # # Run the refinement in parallel for all EMDB-PDB pairs
    # _ = Parallel(n_jobs=10, verbose=10)(
    #     delayed(run_servalcat_iterative)(
    ...
    #         verbose = True,
    #     ) for emdb_pdb in tqdm(emdb_pdbs_present, desc="Processing Model Based")
    # )

    # _ = Parallel(n_jobs=10, verbose=10)(
    #     delayed(run_servalcat_iterative)(
    #         model_path = input_files[emdb_pdb]["target_pdb_path"],
    #         map_path = [\
    #             input_files[emdb_pdb]["halfmap_1_path"],\
    #             input_files[emdb_pdb]["halfmap_2_path"]\
    #         ],
    #         resolution = input_files[emdb_pdb]["resolution"],
    #         num_iter = 10, 
    #         pseudomodel_refinement = True, 
    #         verbose = True,
    #     ) for emdb_pdb in tqdm(emdb_pdbs_present, desc="Processing Model Free")
    # )
    _ = Parallel(n_jobs=10, verbose=10)(
        delayed(run_servalcat_iterative)(
            model_path = input_files_hyb[emdb_pdb]["target_pdb_path"],
            map_path = [\
                input_files_hyb[emdb_pdb]["halfmap_1_path"],\
                input_files_hyb[emdb_pdb]["halfmap_2_path"]\
            ],
            resolution = input_files_hyb[emdb_pdb]["resolution"],
            num_iter = 10, 
            hybrid_model_refinement = True, 
            verbose = True,
            final_chain_counts = [1]
        ) for emdb_pdb in tqdm(emdb_pdbs_present, desc="Processing Hybrid")
    )
