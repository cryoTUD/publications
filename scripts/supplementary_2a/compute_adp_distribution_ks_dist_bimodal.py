## IMPORTS
import os
import sys
sys.path.append(os.environ["LOCSCALE_2_SCRIPTS_PATH"])
sys.path.append("/home/abharadwaj1/dev/locscale")
import warnings
warnings.filterwarnings("ignore")
import numpy as np
import random
import pickle
import json
from tqdm import tqdm
from scipy.stats import invgamma
from scipy.signal import find_peaks
from locscale.include.emmer.pdb.pdb_utils import get_bfactors
from scripts.utils.general import setup_environment, assert_paths_exist, create_folders_if_they_do_not_exist
from s2a_utils import get_atomic_bfactor_correlation, get_input_files_for_adp_correlation_atomicmodel_pseudomodel
from joblib import Parallel, delayed
import pandas as pd
from locscale.include.emmer.pdb.pdb_utils import get_bfactors, shift_bfactors_by_probability

np.random.seed(42)
random.seed(42)
# PARAMETERS
# Set global variables
EMDB_PDB_ids_training = ["0026_6gl7", "7573_6crv",  "0665_6oa9", "0038_6gml", "0071_6gve", "0093_6gyn", "0094_6gyo", "0132_6h3c", "0234_6hjn", "0408_6nbd", "0415_6nbq", "4288_6fo2", "0452_6nmi", "0490_6nr8", "0492_6nra", "0567_6o0h", "0589_6nmi", "0592_6o1m", "0776_6ku9", "10049_6rx4", "10069_6s01", "10100_6s5t", "10105_6s6t", "10106_6s6u", "10273_6sof", "10279_6sp2", "10324_6swe", "10333_6swy", "10418_6t9n", "10534_6tni", "10585_6ttu", "10595_6tut", "10617_6xt9", "20145_6oo4", "20146_6oo5", "20189_6osy", "20234_6p19", "20249_6p4h", "20254_6p5a", "20259_6p62", "20270_6p7v", "20271_6p7w", "20352_6pik", "20521_6pxm", "20986_6v0b", "21012_6v1i", "21107_6v8o", "21144_6vbu", "21391_6vv5", "3661_5no2", "3662_5no3", "3802_5of4", "3885_6el1", "3908_6eoj", "4032_5lc5", "4073_5lmn", "4074_5lmo", "4079_5lmt", "4148_5m3m", "4162_6ezo", "4192_6f6w", "4214_6fai", "4241_6fe8", "4272_6fki", "4401_6i2x", "4404_6i3m", "4429_6i84", "4588_6qm5", "4589_6qm6", "4593_6qma", "4728_6r5k", "4746_6r7x", "4759_6r8f", "4888_6ric", "4889_6rid", "4890_6rie", "4907_6rkd", "4917_6rla", "4918_6rlb", "4941_6rn3", "4983_6rqj", "7009_6ave", "7041_6b3q", "7065_6b7y", "7090_6bf6", "7334_6c23", "7335_6c24", "8911_6dt0", "8958_6e1n", "8960_6e1p", "9258_6muw", "9259_6mux", "9931_6k7g", "9934_6k7i", "9935_6k7j", "9939_6k7l", "9941_6k7m", "9695_6iok"]
EMDB_PDB_ids_validation = ["0193_6hcg", "0257_6hra", "0264_6hs7", "0499_6nsk", "10401_6t8h", "20449_6pqo", "20849_6uqk", "4611_6qp6", "4646_6qvb", "4733_6r69", "4789_6rb9", "7133_6bqv", "7882_6dg7", "8069_5i08", "9112_6mgv", "9298_6mzc", "9374_6nhv"]
EMDB_PDB_ids_epsilon = ["0282_6huo", "0311_6hz5", "0560_6nzu", "10365_6t23", "20220_6oxl", "20226_6p07", "3545_5mqf", "4141_5m1s", "4531_6qdw", "4571_6qk7", "4997_6rtc", "7127_6bpq",  "8702_5vkq", "9610_6adq"]
EMDB_PDB_ids_all = EMDB_PDB_ids_training + EMDB_PDB_ids_validation + EMDB_PDB_ids_epsilon

res_dict = {"0026" : 6.3, "0038" : 3.2, "0071" : 3.9, "0093" : 3.4, "0094" : 3.4, "0132" : 3.9, "0234" : 3.3, "0408" : 3.2, "0415" : 3.1, "4288" : 4.4, "0452" : 3.7, "0490" : 7.8, "0492" : 7.7, "0567" : 3.67, "0589" : 3.9, "0592" : 3.15, "0665" : 3.9, "0776" : 2.67, "10049" : 3.3, "10069" : 3.2, "10100" : 4.15, "10105" : 4.1, "10106" : 3.5, "10273" : 4.3, "10279" : 3.33, "10324" : 3.1, "10333" : 3.2, "10418" : 2.96, "10534" : 3.4, "10585" : 3.7, "10595" : 3.25, "10617" : 3.8, "20145" : 3.3, "20146" : 4.2, "20189" : 4.3, "20234" : 3.8, "20249" : 3.2, "20254" : 3.6, "20259" : 3.57, "20270" : 4, "20271" : 4.1, "20352" : 7.8, "20521" : 2.1, "20986" : 4.1, "21012" : 3.8, "21107" : 3.07, "21144" : 3.1, "21391" : 3.5, "3661" : 5.16, "3662" : 5.16, "3802" : 4.4, "3885" : 6.1, "3908" : 3.55, "4032" : 4.35, "4073" : 3.55, "4074" : 4.3, "4079" : 4.15, "4148" : 4, "4162" : 4.1, "4192" : 3.81, "4214" : 3.4, "4241" : 4.1, "4272" : 4.3, "4401" : 3.35, "4404" : 3.93, "4429" : 4.4, "4588" : 3.6, "4589" : 3.7, "4593" : 3.7, "4728" : 4.8, "4746" : 3.47, "4759" : 3.8, "4888" : 2.8, "4889" : 2.9, "4890" : 3.1, "4907" : 3.2, "4917" : 3.9, "4918" : 4.5, "4941" : 4, "4983" : 3.5, "7009" : 3.7, "7041" : 3.7, "7065" : 6.5, "7090" : 6.5, "7334" : 3.9, "7335" : 3.5, "8911" : 3.7, "8958" : 3.7, "8960" : 3.7, "9258" : 3.6, "9259" : 3.9, "9931" : 3.3, "9934" : 3.22, "9935" : 3.08, "9939" : 2.83, "9941" : 2.95, "9695" : 3.64, "0193" : 4.3, "0257" : 3.7, "0264" : 4.6, "0499" : 2.7, "10401" : 3.77, "20449" : 2.88, "20849" : 3.77, "4611" : 3.2, "4646" : 4.34, "4733" : 3.65, "4789" : 3.2, "7133" : 3.1, "7882" : 3.32, "8069" : 4.04, "9112" : 3.1, "9298" : 4.5, "9374" : 3.5, "0282" : 3.26, "0311" : 4.2, "0560" : 3.2, "10365" : 3.1, "20220" : 3.5, "20226" : 3.2, "3545" : 5.9, "4141" : 6.7, "4531" : 2.83, "4571" : 3.3, "4997" : 3.96, "7127" : 4.1, "7573" : 3.2, "8702" : 3.55, "9610" : 3.5}

symmetry_dictionary = {'0026': 'C2', '0038': 'C1', '0071': 'D2', '0093': 'C4', '0094': 'C4', '0132': 'C2', '0234': 'C3', '0408': 'C2', '0415': 'C1', '4288': 'C2', '0452': 'C1', '0490': 'C1', '0492': 'C1', '0567': 'D2', '0589': 'C1', '0592': 'C2', '0665': 'C1', '0776': 'C3', '10049': 'C1', '10069': 'C1', '10100': 'C1', '10105': 'C1', '10106': 'C2', '10273': 'C1', '10279': 'C6', '10324': 'C1', '10333': 'C1', '10418': 'C4', '10534': 'C1', '10585': 'C1', '10595': 'C1', '10617': 'C1', '20145': 'C2', '20146': 'C2', '20189': 'C3', '20234': 'C1', '20249': 'C1', '20254': 'C2', '20259': 'C3', '20270': 'C1', '20271': 'C1', '20352': 'C2', '20521': 'O', '20986': 'C5', '21012': 'C9', '21107': 'C1', '21144': 'C1', '21391': 'C3', '3661': 'C1', '3662': 'C1', '3802': 'C1', '3885': 'C10', '3908': 'C1', '4032': 'C1', '4073': 'C1', '4074': 'C1', '4079': 'C1', '4148': 'C1', '4162': 'C2', '4192': 'C1', '4214': 'C1', '4241': 'C1', '4272': 'C1', '4401': 'C1', '4404': 'C2', '4429': 'C1', '4588': 'C2', '4589': 'C2', '4593': 'C2', '4728': 'C1', '4746': 'C2', '4759': 'C1', '4888': 'C1', '4889': 'C1', '4890': 'C1', '4907': 'D3', '4917': 'C2', '4918': 'C1', '4941': 'C1', '4983': 'C1', '7009': 'C3', '7041': 'C1', '7065': 'C1', '7090': 'C2', '7334': 'C1', '7335': 'C1', '8911': 'C2', '8958': 'C2', '8960': 'C2', '9258': 'C2', '9259': 'C1', '9931': 'C1', '9934': 'C1', '9935': 'C1', '9939': 'C1', '9941': 'C1', '9695': 'C1', '0193': 'C15', '0257': 'C1', '0264': 'C5', '0499': 'C6', '10401': 'C1', '20449': 'C4', '20849': 'C4', '4611': 'C2', '4646': 'C2', '4733': 'C1', '4789': 'C7', '7133': 'C4', '7882': 'C5', '8069': 'C3', '9112': 'C2', '9298': 'C1', '9374': 'C1', '0282': 'C1', '0311': 'C2', '0560': 'C2', '10365': 'C1', '20220': 'C1', '20226': 'C1', '3545': 'C1', '4141': 'C1', '4531': 'C1', '4571': 'C1', '4997': 'C2', '7127': 'C4', '7573': 'C3', '8702': 'C4', '9610': 'C2'}

def statistic(x, y):
    from scipy.stats import kstest
    return kstest(x, y).statistic

# def invgamma_permutation_test(x_observed, x_linear, normalised_pdf):
#     from scipy.stats import permutation_test
#     from scipy.stats import invgamma
#     # print("Running permutation test for inverse gamma distribution...")
#     invgamma_sample_mixture = np.random.choice(x_linear, size=len(x_linear), p=normalised_pdf)
#     # print("Obtained inverse gamma sample mixture for permutation test.")
#     #print(f"length of x_observed: {len(x_observed)}, length of invgamma_sample_mixture: {len(invgamma_sample_mixture)}")
#     perm_test = permutation_test((x_observed,invgamma_sample_mixture), statistic=statistic, n_resamples=1000)
#     # print("Permutation test completed.")
#     #print(perm_test.pvalue)
#     return perm_test
def invgamma_permutation_test(x_observed, x_linear, normalised_pdf):
    from scipy.stats import permutation_test
    from scipy.stats import invgamma
    # print("Running permutation test for inverse gamma distribution...")
    invgamma_sample_mixture = np.random.choice(x_linear, size=len(x_linear), p=normalised_pdf)
    # print("Obtained inverse gamma sample mixture for permutation test.")
    #print(f"length of observed data: {len(x_observed)}")
    #print(f"length of inverse gamma sample mixture: {len(invgamma_sample_mixture)}")
    perm_test = permutation_test((x_observed,invgamma_sample_mixture), statistic=statistic, n_resamples=1000, rng=42)
    # print("Permutation test completed.")
    #print("pvalue:", perm_test.pvalue)
    # view distribution of the null 
    #sns.histplot(perm_test.null_distribution, kde=True)
    # rotate x-axis labels
    #plt.xticks(rotation=45)

    return perm_test
def get_pdf_from_json(json_path, bfactors):
    """
    Given the parameters from the JSON file, return a distribution.
    """
    with open(json_path, 'r') as f:
        params = json.load(f)
    mix = params['Mix_parameters']
    alphas = params['Alpha']
    betas = params['Beta']
    shifts = params['Shift']
    num_modes = len(mix)
    # Create an inverse gamma distribution
    dist_list = []
    X = np.linspace(min(bfactors), max(bfactors), 1000)
    pdfs = np.zeros_like(X)
    for i in range(num_modes):
        dist = invgamma(alphas[i], shifts[i], betas[i])
        pdf = dist.pdf(X)
        pdfs += mix[i] * pdf
    
    # normalise the pdf so that it integrates to 1
   #pdfs /= pdfs.sum()
    
    return X, pdfs

def compute_ks_distance_metric(emdb_pdb, input_files, input_files_MB):
    emdb, pdb = emdb_pdb.split("_")
    atomic_model_path = input_files_MB[emdb_pdb]["target_pdb_path"]
    atomic_json_path = input_files_MB[emdb_pdb]["json"]
    pseudomodel_path = input_files[emdb_pdb]["target_pdb_path"]
    pseudomodel_json_path = input_files[emdb_pdb]["json"]

    atomic_bfactors = get_bfactors(atomic_model_path)
    pseudomodel_bfactors = get_bfactors(pseudomodel_path)

    x_atomic, atomic_pdf = get_pdf_from_json(atomic_json_path, atomic_bfactors)
    x_pseudo, pseudomodel_pdf = get_pdf_from_json(pseudomodel_json_path, pseudomodel_bfactors)

    # Compute the KS distance
    normalised_atomic_pdf = atomic_pdf / np.sum(atomic_pdf)
    normalised_pseudomodel_pdf = pseudomodel_pdf / np.sum(pseudomodel_pdf)

    invgamma_permutation_test_atomic = invgamma_permutation_test(atomic_bfactors, x_atomic, normalised_atomic_pdf)
    invgamma_permutation_test_pseudomodel = invgamma_permutation_test(pseudomodel_bfactors, x_pseudo, normalised_pseudomodel_pdf)
    
    #print(f"{emdb_pdb} - {invgamma_permutation_test_atomic.statistic}, {invgamma_permutation_test_atomic.pvalue}, {atomic_bfactors[:3]}")
    #print(f"{emdb_pdb} - KS distance (pseudomodel): {invgamma_permutation_test_pseudomodel.statistic}, p-value: {invgamma_permutation_test_pseudomodel.pvalue}")
    results = {
        "emdb_pdb" : emdb_pdb,
        "ks_distance_atomic" : invgamma_permutation_test_atomic.statistic,
        "ks_distance_pseudomodel" : invgamma_permutation_test_pseudomodel.statistic,
        "ks_pvalue_atomic" : invgamma_permutation_test_atomic.pvalue,
        "ks_pvalue_pseudomodel" : invgamma_permutation_test_pseudomodel.pvalue,
    }

    return results

def main():
    data_archive_path = setup_environment()
    output_folder_main = os.path.join(data_archive_path, "structured_data", "supplementary_2a")
    folder_containing_json_files = os.path.join(output_folder_main, "tobvalid_analysis", "albe_results")
    assert_paths_exist(output_folder_main, folder_containing_json_files)
    create_folders_if_they_do_not_exist(output_folder_main)
    output_filename = os.path.join(output_folder_main, "ks_distances.csv")  # output plot preferably in json
    ## Get input file paths from the dataset
    
    input_files = {}
    input_files_MB = {}
    emdb_pdbs_present = []
    for emdb_pdb in EMDB_PDB_ids_all:
        emdb, pdb = emdb_pdb.split("_")
        input_files_model_free = get_input_files_for_adp_correlation_atomicmodel_pseudomodel(emdb_pdb, "MF", averagedPseudomodel=True)
        input_files_model_based = get_input_files_for_adp_correlation_atomicmodel_pseudomodel(emdb_pdb, "MB", averagedPseudomodel=False)
        
        required_files = ["target_pdb_path"]
        print("Checking if all required files exist for {}".format(emdb_pdb))
        print("===============================================!")
        # Check if all required files exist and print a warning if not
        missing_files = []
        for required_file in required_files:
            if not os.path.exists(input_files_model_free[required_file]):
                print("\t{} missing".format(required_file), "MF", input_files_model_free[required_file])
                missing_files.append((required_file, "MF"))
            if not os.path.exists(input_files_model_based[required_file]):
                print("\t{} missing".format(required_file), "MB")
                missing_files.append((required_file, "MB"))

        atomic_json_path = os.path.join(folder_containing_json_files, f"PDB_{pdb}_unrefined_shifted_servalcat_refined_servalcat_refined_mixture.json")
        pseudo_json_path = os.path.join(folder_containing_json_files, "pseudomodel", f"emd_{emdb}_FDR_confidence_final_gradient_pseudomodel_proper_element_composition_averaged_mixture.json")
        if len(missing_files) == 0 and os.path.exists(atomic_json_path) and os.path.exists(pseudo_json_path):
            emdb_pdbs_present.append(emdb_pdb)
            input_files[emdb_pdb] = {'json' : pseudo_json_path}
            for key in required_files:
                input_files[emdb_pdb][key] = input_files_model_free[key]
            input_files_MB[emdb_pdb] = {'json' : atomic_json_path}
            for key in required_files:
                input_files_MB[emdb_pdb][key] = input_files_model_based[key]

    print(f"Number of EMDB-PDB pairs: {len(emdb_pdbs_present)}")

    keys_present_in_both = set(input_files.keys()).intersection(set(input_files_MB.keys()))

    results_all = Parallel(n_jobs=10, verbose=10)(
        delayed(compute_ks_distance_metric)(emdb_pdb, input_files, input_files_MB)\
            for emdb_pdb in tqdm(keys_present_in_both, desc="Computing KS distances")\
    )
    # results_all = []
    # for emdb_pdb in tqdm(keys_present_in_both, desc="Computing KS distances"):
    #     try:
    #         result = compute_ks_distance_metric(emdb_pdb, input_files, input_files_MB)
    #         results_all.append(result)
    #     except Exception as e:
    #         print(f"Error processing {emdb_pdb}: {e}")
    #         results_all.append(None)

    results_dict = {x['emdb_pdb']: x for x in results_all if x is not None}
    # convert to following data format
    #   ks_distance_pseudo_values = data["ks_distance_pseudo_values"]
    # ks_distance_atomic_values = data["ks_distance_atomic_values"]
    # ks_pvalues_pseudo_values = data["ks_pvalues_pseudo_values"]
    # ks_pvalues_atomic_values = data["ks_pvalues_atomic_values"]
    # emdb_pdbs = data["EMDB_PDB_ids_present"]

    ks_distance_pseudo_values = [x['ks_distance_pseudomodel'] for x in results_dict.values()]
    ks_distance_atomic_values = [x['ks_distance_atomic'] for x in results_dict.values()]
    ks_pvalues_pseudo_values = [x['ks_pvalue_pseudomodel'] for x in results_dict.values()]
    ks_pvalues_atomic_values = [x['ks_pvalue_atomic'] for x in results_dict.values()]
    emdb_pdbs = list(results_dict.keys())

    # Save the results in a dictionary
    results_dict = {
        "ks_distance_pseudo_values": ks_distance_pseudo_values,
        "ks_distance_atomic_values": ks_distance_atomic_values,
        "ks_pvalues_pseudo_values": ks_pvalues_pseudo_values,
        "ks_pvalues_atomic_values": ks_pvalues_atomic_values,
        "EMDB_PDB_ids_present": emdb_pdbs,
        "information": "The values of KS distance and p-values are found using permutation tests with 1000 resample using the KS statistic"
    }

    # Save the results to a pandas dataframe
    df = pd.DataFrame({
        "ks_distance_pseudo_values": ks_distance_pseudo_values,
        "ks_distance_atomic_values": ks_distance_atomic_values,
        "ks_pvalues_pseudo_values": ks_pvalues_pseudo_values,
        "ks_pvalues_atomic_values": ks_pvalues_atomic_values,
        "EMDB_PDB_ids_present": emdb_pdbs
    })
    # Save the results to a csv file
    df.to_csv(output_filename, index=False)

if __name__ == "__main__":
    main()
