...
from s2a_utils import compute_fsc_curve_emdb_pdb, get_input_files_for_adp_correlation_atomicmodel_pseudomodel
...
def main():
    ...
    output_folder_main = os.path.join(data_archive_path, "processed", "structured_data", "figure_2")
    create_folders_if_they_do_not_exist(output_folder_main)
    output_filename = os.path.join(output_folder_main, "fsc_curves_test_83.pkl")  # output plot preferably in json
    ...
    input_files_hyb = {}
    emdb_pdbs_present = []
    check_emdb_id = ['0592_6o1m', '0234_6hjn', '0560_6nzu', '10333_6swy']
    # check if all check_emdb_ids are present in EMDB_PDB_ids_all
    for emdb_pdb in check_emdb_id:
        if emdb_pdb not in EMDB_PDB_ids_all:
            print(f"{emdb_pdb} not found in EMDB_PDB_ids_all")
            return
        
    for emdb_pdb in EMDB_PDB_ids_all:
        input_files_model_free = get_input_files_for_adp_correlation_atomicmodel_pseudomodel(emdb_pdb, "MF")
        input_files_model_based = get_input_files_for_adp_correlation_atomicmodel_pseudomodel(emdb_pdb, "MB")
        input_files_hybrid = get_input_files_for_adp_correlation_atomicmodel_pseudomodel(emdb_pdb, "hybrid")

        required_files = ["unsharpened_map_file", "model_map_path", "mask_path"]
        ...
        if emdb_pdb in check_emdb_id:
            # print what the status of the files is 
            print(f"Checking {emdb_pdb} for required files:")
            for required_file in required_files:
                mf_status = input_files_model_free[required_file] is not None
                mb_status = input_files_model_based[required_file] is not None
                hybrid_status = input_files_hybrid[required_file] is not None
                print(f"\t{required_file}: MF={mf_status}, MB={mb_status}, hybrid={hybrid_status}")

                mf_present = os.path.exists(input_files_model_free[required_file]) if input_files_model_free[required_file] else False
                mb_present = os.path.exists(input_files_model_based[required_file]) if input_files_model_based[required_file] else False
                hybrid_present = os.path.exists(input_files_hybrid[required_file]) if input_files_hybrid[required_file] else False
                print(f"\t{required_file} paths: MF={mf_present}, MB={mb_present}, hybrid={hybrid_present}")
            
        missing_files = []
        for required_file in required_files:
            if input_files_model_free[required_file] is None or input_files_model_based[required_file] is None or input_files_hybrid[required_file] is None:
                #print("\t{} not found in input files".format(required_file))
                missing_files.append((required_file, "MF/MB/hybrid"))
                continue
            ...
            if not os.path.exists(input_files_hybrid[required_file]):
                #print("\t{} missing".format(required_file), "hybrid")
        ...
        if len(missing_files) == 0:
            ...
            input_files_hyb[emdb_pdb] = {}
            for key in required_files:
                input_files_hyb[emdb_pdb][key] = input_files_hybrid[key]
                
    print(f"Number of EMDB-PDB pairs: {len(emdb_pdbs_present)}")
    print(f"first 10 mf files: {list(input_files.keys())[:10]}")
    print(f"first 10 mb files: {list(input_files_MB.keys())[:10]}")
    print(f"first 10 hybrid files: {list(input_files_hybrid.keys())[:10]}")
    keys_present_in_all = set(input_files.keys()).intersection(set(input_files_MB.keys())).intersection(set(input_files_hyb.keys()))
    print(f"Number of EMDB-PDB pairs with all files: {len(keys_present_in_all)}")
    fsc_curves_combined = []
    
    # check if check_emdb_id are present in keys_present_in_all
    for emdb_pdb in check_emdb_id:
        if emdb_pdb not in keys_present_in_all:
            print(f"{emdb_pdb} not found in keys_present_in_all")
            return
        else:
            print(f"{emdb_pdb} found in keys_present_in_all")

    # parallelise the FSC curve computation
    fsc_curves_combined = Parallel(n_jobs=10, verbose=10)(
        delayed(compute_fsc_curve_emdb_pdb)(input_files[emdb_pdb], input_files_hyb[emdb_pdb], input_files_MB[emdb_pdb], emdb_pdb)
        for emdb_pdb in tqdm(keys_present_in_all)
    )

    # Save to pickle
    with open(output_filename, "wb") as f:
        pickle.dump(fsc_curves_combined, f)

    print(f"Saved neighborhood correlation data to {output_filename}")
